...
6. If using GitHub Actions, the workflow in `.github/workflows/azure-static-web-apps.yml` builds locally, uploads the `.next` folder (`skip_app_build: true`), and forces Node 20 via `NODE_VERSION` so Oryx doesn’t run inside the action with an unsupported runtime.
