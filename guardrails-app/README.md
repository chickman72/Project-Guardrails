...
6. If using GitHub Actions, the workflow in `.github/workflows/azure-static-web-apps.yml` builds locally and then tells the Azure action to upload the pre-built `.next` folder (`skip_app_build: true`), so Oryx doesn’t run inside the action and the Node version constraint is satisfied.
